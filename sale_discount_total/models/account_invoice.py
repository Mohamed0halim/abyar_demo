...
class AccountInvoice(models.Model):
    ...
    def get_credit(self):
        for rec in self:
            if rec.partner_id and rec.acustom_customer_balance == 0:
                ...
                # print('credit', credit)
                rec.acustom_customer_balance = debit - credit
                # print('rec.acustom_customer_balance', rec.acustom_customer_balance, debit, credit)
                if rec.currency_id:
                    ...
                    # else:
                    #     net_balance = rec.acustom_customer_balance / rec.currency_id.rate_ids[0].company_rate
                    #     rec.acustom_customer_balance = net_balance
            else:
                rec.acustom_customer_balance = rec.acustom_customer_balance or 0

    @api.depends('partner_id', 'state', 'currency_id')
    def get_final_customer_balance(self):
        for rec in self:
            if rec.partner_id and rec.final_customer_balance == 0 and rec.state == 'posted':
                account_move_line = self.env['account.move.line'].sudo().search(
                    [('partner_id', '=', rec.partner_id.id),
                     ('parent_state', '=', 'posted'),
                     ('account_id.internal_type', 'in', ('receivable', 'payable')),
    ...
    acustom_customer_balance = fields.Float(string="Previous Balance", required=False, readonly=True,
                                            compute='get_credit', store=True, copy=False,
                                            tracking=True)
    ...
    def get_last_customer_balance_from_last_inv(self):
        for rec in self:
            rec.last_customer_balance_from_last_inv = 0
            if rec.partner_id:
